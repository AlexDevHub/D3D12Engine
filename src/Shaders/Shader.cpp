...
HRESULT Shader::UpdateTime(ID3D11DeviceContext *device_context) {
    // // Lock the constant buffer so it can be written to.
    // D3D11_MAPPED_SUBRESOURCE mappedResource = {};
    // RETURN_FAIL_IF_FAILED(device_context->Map(m_timeBuffer.Get(), 0, D3D11_MAP_WRITE_DISCARD, 0, &mappedResource))
    //
